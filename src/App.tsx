...
type ErrorState = { kind: string; message: string } | null;

const formatTimeUntilReset = (resetDateString: string) => {
...
const CacheIndicator = ({
  ...
  if (!cached && diffMins < 1) {
    return (
      <div className="flex items-center gap-1.5 text-[11px] text-emerald-700 font-medium bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">
  ...
  return (
    <div className="flex min-h-svh flex-col items-center justify-center gap-4 p-8">
      <Button onClick={fetchUsage} disabled={loading}>
        {loading ? 'Loading\u2026' : 'Fetch Claude usage'}
      </Button>

      {error && (
        <div
          className={`w-full max-w-md rounded-lg border p-3 text-sm ${
